...
int CrashDetectUtils::DetectCircleAndCircle(CGPoint ptOrg, float nOrgR,  CGPoint ptDest, float nDestR)
{
	float distanceX = fabs(fabs(ptOrg.x) - fabs(ptDest.x)); 
	float distanceY = fabs(fabs(ptOrg.y) - fabs(ptDest.y)); 
	float sumR = nOrgR + nDestR;
	if (distanceX < sumR && distanceY < sumR)
		return CRASH_YES;
 	return CRASH_NO;
}
